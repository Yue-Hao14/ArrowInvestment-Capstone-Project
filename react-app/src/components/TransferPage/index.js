          ...
          transfers.map(transfer => (
            <div className='individual-transfer-container'>
              ...
              <div className={transfer.amount >0 ? "individual-transfer-amount green": "individual-transfer-amount red"}>
                {transfer.amount > 0 ? `$${transfer.amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : `-$${Math.abs(transfer.amount).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
                </div>
            </div>
          ))
